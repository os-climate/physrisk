...
\paragraph{Hazard.}
As noted above, hazard models come in two varieties: models of acute hazards -- events -- and models of chronic hazards -- long-term shifts in climate parameters.
...
\emph{Vulnerability models} are applied to different types of asset and different types of hazard. A vulnerability model will use information about an asset, typically its geolocation, type and relevant characteristics, and will source hazard indicator data from one or more \emph{hazard models}. The vulnerability model will construct from these inputs an impact distribution.  
...
A typical physical risk calculation will generate an impact, $d$\footnote{$d$ for `damage/disruption' is used to denote impact as $i$ is reserved for indexing}. 
...
The impact of hazards on a portfolio of $n$ assets is a (multivariate) probability distribution as a result of the primary and secondary uncertainties. The impact on a single asset is a random variable, $D$, with (marginal) probability density function $f_D(d)$. The probability of an impact exceeeding $d$, $F_D(d)$ is given by:
...
The value of a hazard indicator, which quantifies the intensity of the phenomenon to which the hazard refers (we also use the term `hazard intensity') is given by random variable $S$. This has marginal  probability density function $f_S(s)$. This distribution captures the primary uncertainty.
...
Note that $f_d(d)$ is identical to the {\it effective damageability} distribution of Oasis LMF\cite{OasisFinancialModule} and can be described as the `effective impact'. It is a marginal distribution and as such does not capture any correlation between events nor impacts.

In the case of $n$ assets, multivariate random variables are defined $\mathbf{D} = (D_1,\ldots,D_d)$ and $\mathbf{S} = (S_1,\ldots,S_n)$. 

\paragraph{Return-period-based approach.}

The \emph{return-period-based approach} makes use of marginal distributions $f_{S_i}$ for each (asset) location $i$, and not the joint distribution, $f_S(\mathbf{s})$. The marginals are related to the joint distribution by: 
...
Equation~\ref{Eq:ImpactEffective} can be applied to obtain the impact distributions for each location $i$. In order to aggregate the impacts, the dependency structure must be provided; more precisely, a copula\cite{Nelsen:2007} must be specified. This may be derived via a heuristic; for example a `worst case' dependency structure could be provided to obtain an upper bound, perhaps assuming 100\% correlation between losses. In general such approaches are not intended for cases where accurate treatment of the dependence between the losses is needed, but rather for:

\begin{itemize}
\item analyses of single assets
\item quick calculations intended to give an upper bound of an impact, perhaps to identify areas of particular risk
\item certain calculations where intra-correlation is not required, e.g. expected total ground-up loss
\end{itemize}

In order to apply  $f_{D}(\mathbf{d})$, Monte Carlo approaches can be employed.

principle, one can adopt first sample from the joint distribution $f_{D, S}(\mathbf{d}, \mathbf{s})$. 


\paragraph{Event-based approach.}
...
We define $v^{(a, b)}_{ij}$ to be the conditional probability that \emph{given} the occurrence of an event of type $a$ with intensity $S^{(a)}$ there is an impact (typically a damage or disruption, $D^{(b)}$ in the range $d^{(a,b,\text{lower})}_j < D \le d^{(a,b,\text{upper})}_j$. The impact is of type $b$.
