...
\subsection{Threshold-based vulnerability models}
It is possible to construct a framework for implementing a variety of chronic vulnerability models, including those defined in \cite{LuoEtAl:2021}. In contrast to acute model hazard indicators -- which comprise quantities (e.g. flood depth or wind speed) for different return periods -- the chronic hazard indicators are based on thresholds. Examples are:

\begin{itemize}
    \item number of days in a year with near-surface air temperature above temperature $x$ where $x$ is 25$^\circ$C, 30$^\circ$C, 35$^\circ$C etc.
    \item number of months in a year with SPEI \cite{VicenteSerranoEtAl:2010} below $x$ where $x$ is -1 (mild drought), -1.5 (moderate drought), -2 (severe drought) etc.
    \item number of annual degree days above $x$ where $x$ is 18$^\circ$C, 20$^\circ$C, 22$^\circ$C etc.
\end{itemize}
...
Vulnerability models were described in \ref{SubSec:VulnerabilityModel} as a means to calculate the damage and disruption to a portfolio of assets for different hazard models. The damage is given as a fraction of the total insurable value (TIV) of the asset. The disruption is given as a fraction of the revenue generation that is attributable to the asset. How the resulting probability distributions of impact are then used in a financial model depends on the intent of the analysis. We distinguish between \emph{scenario analyses} and \emph{operational monitoring} use cases.

\emph{Scenario analyses}, of which stress tests are an example, make use of climate-related shocks in order to assess financial impact. This might be the impact on a financial institution. Stress tests may make use of narrative scenarios or may take the approach of simulating large number of extreme but plausible events in order to identify a worst-case which is \emph{a priori} unknown. Indeed the two approaches may be combined, assessing the impact of a severe acute event coupled with other aggravating factors.

\emph{Operational monitoring}, can take several forms, for example assessing which parts of a portfolio of assets are most subject to physical climate risk. Or identifying \emph{ex ante} if a particular asset is subject to elevated risk.
