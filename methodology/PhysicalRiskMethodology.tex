...
The continuous forms of expressions for the impact distribution are given by Equation~\ref{Eq:ImpactMarginal2} and Equation~\ref{Eq:Severity2}, the latter being the special case of deterministic hazard -- general encountered for one specific event. Here a discrete version of Equation~\ref{Eq:ImpactMarginal2} is defined.

There are $n_b$ intensity bins with index $q$ such that $q \in \{1, \dots, n_b \}$.  $\sigma^{(h)}_q$ is defined to be the probability that a hazard event of type $h$ occurs in a given year with an intensity that falls in bin $q$. With random variable $S^{(h)}$, now with superscript, being the intensity of hazard type $h$:
...
We define $\delta^{(h,b)}_q$ to be the marginal probability of impact $D^{(b)}$ in the range $d^{(b, \text{lower})}_q < D^{(b)} \le d^{(b,\text{upper})}_q$ occurring as a result of an event of type $a$.

\begin{equation}
    \label{Eq:impact}
    \delta^{(b)}_q =  \mathbb{P} \left[ d^{(b,\text{lower})}_q < D^{(b)} \le d^{(b,\text{upper})}_q \right]
\end{equation}
...
\begin{equation}
    \label{Eq:model}
    \delta^{(b)}_p = \sum_{i} v^{(h,b)}_{pq} \sigma^{(h)}_q
\end{equation}
...
The Monte Carlo sampling is done by constructing a cumulative probability density function, $Y_D(d)$, of impact $D$ from the effective impact distribution ($Y_D(d) = \mathbb{P}[D \le d$]). Random numbers, $u_i$ are then sampled from a standard uniform distribution ($u_i \in [0, 1]$), from which impacts are calculated by:
