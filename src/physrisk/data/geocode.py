...
class Geocoder:
    ...
    def get_countries(self, latitudes: Sequence[float], longitudes: Sequence[float]):
        gdf = gpd.GeoDataFrame(crs=self.crs, geometry=gpd.points_from_xy(longitudes, latitudes)).to_crs(crs=3857)
    ...
    def get_continent_and_country_from_code_iso_3166(country_codes: Sequence[Union[str, int]]) -> pd.DataFrame:
        with importlib.resources.path(physrisk.data.ne_110m_admin_0_countries, "country_codes.tsv") as path:
            ...
            if (len(numeric_codes) + len(alpha2_codes) + len(alpha3_codes)) != len(country_codes):
                raise ValueError("invalid country codes")
            return pd.concat(
                [
                    df[df["Numeric"].isin(numeric_codes)][["Numeric", "Country", "Continent"]].rename(
                        columns={"Numeric": "Code"}
                    ),
                    df[df["Alpha-2 code"].isin(alpha2_codes)][["Alpha-2 code", "Country", "Continent"]].rename(
                        columns={"Alpha-2 code": "Code"}
                    ),
                    df[df["Alpha-3 code"].isin(alpha3_codes)][["Alpha-3 code", "Country", "Continent"]].rename(
                        columns={"Alpha-3 code": "Code"}
                    ),
                ]
