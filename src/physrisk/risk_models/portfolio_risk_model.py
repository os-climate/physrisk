...
from physrisk.api.v1.impact_req_resp import Category, RiskMeasureDefinition, ScoreBasedRiskMeasureDefinition
...
class AveragingAssetBasedPortfolioRiskMeasureCalculator:
    ...
    def calculate_risk_measures(
        ...
        measure_by_year_scen: dict[tuple[str, int|None], list[MeasureKey]] = defaultdict(list)
        ...
        for k, v in measure_by_year_scen.items():
            ...
            portfolio_measures[MeasureKey(None, k[0], k[1], None)] = Measure(score=Category(round(average_score)), measure_0=average_score, definition=self._definition())
