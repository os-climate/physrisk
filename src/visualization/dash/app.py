...
asset_filt = asset_filt.append(
    {
        ...
        "latitude": 2.898,
        "capacity_mw": 102,
        "estimated_generation_gwh": 394.4,
    },
    ignore_index=True,
)
    ...
    wri_tile = dl.Overlay(
        children=[
            dl.TileLayer(
                ...
                attribution="Riverine inundation",
            )
        ],
        name="Riverine inundation",
    )

    cl = dl.Overlay(
        children=[dl.GeoJSON(data=dlx.dicts_to_geojson(markers), cluster=True, id="markers")],
        id="clusters",
        name="Power generating assets (gas)",
    )

    map = dl.Map(
        dl.LayersControl([dl.BaseLayer(dl.TileLayer(), name="Base Layer"), wri_tile, cl]),
        id="the-map",
        preferCanvas=True,
        center=[39, -98],
        zoom=4,
...
def create_map_fig(assets, layers):
    ...
    map_fig.layout.mapbox.layers = [
        {"below": "traces", "sourcetype": "raster", "source": [url], "sourceattribution": "WRI", "visible": True}
    ]

    update_map_fig_layers(map_fig, layers)

    map_fig_colorbar = make_subplots(rows=1, cols=1)
    _, min_index = color_map_info["min"]["data"], color_map_info["min"]["color_index"]
    _, max_index = color_map_info["max"]["data"], color_map_info["max"]["color_index"]
    colorscale = []
    ...
    colorbar = go.Heatmap(
        x=levels, y=[0.0, 1.0], z=[levels[:-1]], colorscale=colorscale, hoverongaps=False, hovertemplate=""  # 'Reds'
    )

    map_fig_colorbar.add_trace(colorbar, row=1, col=1)

    map_fig_colorbar.update_layout(
        height=60,
...
def get_fig_for_asset(asset, detailed_results):
    ...
    fig.add_trace(go1, row=1, col=1)

    fig.add_trace(go2, row=1, col=2)
...
def get_fig_gos_for_asset(asset, detailed_results):
    ...
    go1 = go.Bar(
        x=0.5 * (impact_bins[0:-1] + impact_bins[1:]),
        y=impact_bin_probs * 100,
        width=impact_bins[1:] - impact_bins[:-1],
    )
...
def get_fig_gos_for_model(asset, detailed_results):
    ...
    go1 = go.Bar(
        x=0.5 * (intensity_bins[0:-1] + intensity_bins[1:]),
        y=intensity_bin_probs * 100,
        width=intensity_bins[1:] - intensity_bins[:-1],
    )

    exc = res.event.exceedance  # to_exceedance_curve()
    go2 = go.Scatter(x=exc.probs * 100, y=exc.values)

    headline = "10% probability of event with intensity greater than {0:.2f}m in a single year".format(
        exc.get_value(0.1)
    )
    ...
    children=[
        html.Div(
            children=[
                ...
                html.H1(children="Asset loss drill-down", className="header-title"),
                html.P(
                    children="Drill into financial loss" " at asset level: loss, hazard intensity" " and vulnerability",
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            id="asset-filter",
                            options=[{"label": asset, "value": asset} for asset in np.sort(asset_categories)],
                ...
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            id="hazard-filter",
                            options=[{"label": hazard, "value": hazard} for hazard in np.sort(hazards)],
                            value="Inundation",
                            clearable=False,
                            searchable=False,
                            className="dropdown",
                ...
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            id="dates",
                            options=[{"label": date, "value": date} for date in ["Today", "2030", "2050", "2080"]],
                            multi=True,
                            value=["2080"],
                            clearable=False,
                            searchable=False,
                ...
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            id="scenario-filter",
                            options=[
                                {"label": scenario, "value": scenario} for scenario in ["Histo", "RCP4.5", "RCP8.5"]
                            ],
                            multi=True,
                            value=["RCP8.5"],
                            clearable=False,
        ...
        html.Div(
            children=[
                ...
                html.Div(
                    children=[
                        dcc.Graph(id="map-chart", figure=map_fig, style={"width": "100%", "height": "40vh"}),
                    ],
                    style={"margin": "2px"},
                    className="card",
                ),
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            children=dash_table.DataTable(
                                ...
                                columns=[
                                    ...
                                    dict(
                                        id="estimated_generation_gwh",
                                        name="Est. annual Generation (GWh)",
                                        type="numeric",
                        ...
                        html.Div(
                            children=dcc.Graph(id="exceedance-chart", config={"displayModeBar": False}),
                            className="card",
                        ),
                        html.Div(
                            children=dcc.Graph(id="intensity-chart", config={"displayModeBar": False}),
                            className="card",
                        ),
                        html.Div(
                            children=dcc.Graph(id="vulnerability-chart", config={"displayModeBar": False}),
                            className="card",
                        ),
                    ],
                ),
...
@app.callback(
    # [Output('drilldown', 'style'),
    [
        Output("map-chart-colorbar-container", "style"),
        Output("map-chart", "figure"),
    ],  # , Output('map-chart-colorbar', 'figure')],
...
def update_map_layers(value, fig):
    ...
    map_fig.update_layout(mapbox_center_lon=lon, mapbox_center_lat=lat, mapbox_zoom=zoom)
