...
from physrisk.api.v1.common import Assets, Asset
...
def test_impact_request():
    ...
    assert response.asset_impacts[0].impacts[0].hazard_type == "CoastalInundation"
...
def test_thermal_power_generation():
...
def test_example_portfolios(test_dir, load_credentials):
    ...
    for assets in example_portfolios:
        ...
        with open(os.path.join(test_dir, "out.json"), "w") as f:
            f.write(response)
        assert response is not None


@pytest.mark.skip(reason="Example, not test. And requires credentials")
def test_example_portfolios_risk_measures(load_credentials):
    assets = {
        "items": [
            {
                "asset_class": "RealEstateAsset",
