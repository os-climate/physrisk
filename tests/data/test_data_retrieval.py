...
def test_inventory_change(tmp_path):
    ...
    with open(
        os.path.join(tmp_path, "inventory.json"),
        "w",
    ) as f:
        f.write(inventory)
...
@pytest.mark.skip(reason="Requires write credentials.")
def test_set_get_inventory_s3():
...
def test_cascading_sources_and_interpolation():
