...
async def test_geocode_candidate_count_reflects_multiple_results():
    session = _mock_session(address_payload=_AMBIGUOUS_ADDRESS_RESPONSE)
    async with GoogleGeocoder("test-key", session=session) as geocoder:
        result = await geocoder.geocode("High Street")

    assert result is not None
    assert result.candidate_count == 2
    # first candidate is returned
    assert result.formatted_address == "High Street, London, UK"
...
async def test_geocode_many_respects_max_concurrency():
    ...
    async def tracking_fetch(self_inner, address, region_code):
        nonlocal active, peak
        active += 1
        peak = max(peak, active)
        await asyncio.sleep(0)  # yield so other tasks can interleave
        result = await orig_fetch(self_inner, address, region_code)
        active -= 1
        return result
...
async def test_geocode_per_request_region_code_used_when_no_instance_default():
    session = _mock_session()
    async with GoogleGeocoder("test-key", session=session) as geocoder:
        await geocoder.geocode("10 Downing Street, London", region_code="GB")
...
async def test_geocode_many_region_codes_length_mismatch_raises():
    session = _mock_session()
    async with GoogleGeocoder("test-key", session=session) as geocoder:
        with pytest.raises(ValueError, match="region_codes length"):
            await geocoder.geocode_many(["A", "B", "C"], region_codes=["FR", "DE"])
