...
def test_standard_deviations():
    ...
    assert stddev == pytest.approx(stddev_exp)
    ...
    semi_stddev_exp += ((1.0 - mean) ** 3 - (0.5 - mean) ** 3) * 0.2 / (3 * (1.0 - 0.5))
    semi_stddev_exp += (
        ((1.5 - mean) ** 3 - (1.0 - mean) ** 3) * 0.15 / (3 * (1.5 - 1.0))
    )
    semi_stddev_exp = np.sqrt(semi_stddev_exp)

    assert semi_stddev == pytest.approx(semi_stddev_exp)
    ...
    # assert stddev == pytest.approx(semi_stddev_exp)
    semi_stddev = impact.semi_standard_deviation()
    assert semi_stddev == pytest.approx(semi_stddev_exp)
