...
def test_jupiter_exposure_service(get_components):
    assets, store, hazard_model_factory, expected, vulnerability_models_factory = (
        get_components
    )
    ...
    requester = Requester(
        ...
        inventory=inventory,
        resource_provider=get_default_source_paths(inventory),
...
def test_jupiter_exposure(get_components):
    assets, _, hazard_model_factory, expected, _ = get_components
    asset = assets[0]
...
def get_components():
    ...
    paths = [
        "combined_flood/jupiter/v1/fraction_{scenario}_{year}",  # There is no fraction data in this path
    ...
    # Fire Category was LOW before, now it's HIGHEST after the rebase
    expected = {
        ...
        # Fire: Category.LOW, # Fire Category was LOW before, now it is HIGHEST after the rebase
    ...
    assets = [
        Asset(lat, lon) for (lat, lon) in zip(TestData.latitudes, TestData.longitudes)
    ]
    ...
    vulnerability_models_factory = DictBasedVulnerabilityModelsFactory()

    return assets, store, hazard_model_factory, expected, vulnerability_models_factory
